...
#+begin_src emacs-lisp
  ...
  (setq which-key-posframe-parameters '((left-fringe . 8)
                                        (right-fringe . 8)))
...
#+begin_src elisp
  ...
  (with-eval-after-load 'corfu
    ;; 取消 C-n / C-p 在补全状态下的绑定，让它们始终向下/向上移动光标
    (define-key corfu-map (kbd "C-n") nil)
    (define-key corfu-map (kbd "C-p") nil))
...
#+begin_src emacs-lisp
  ...
  (add-to-list 'jinx-exclude-regexps '(t "\\p{Han}"))
